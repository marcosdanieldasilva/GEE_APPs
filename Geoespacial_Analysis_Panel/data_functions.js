...
exports.compositeItems = [
  ...
  {label: 'Create a Mosaic', value: {label: 'Mosaic', value: function(collection) { return collection.mosaic(); }}},
  ...
  {label: 'Composite Median', value: {label: 'Median', value: function(collection) { return collection.median(); }}},
  {label: 'Composite Mean', value: {label: 'Mean', value: function(collection) { return collection.mean(); }}},
  {label: 'Composite Min', value: {label: 'Min', value: function(collection) { return collection.min(); }}},
  {label: 'Composite Max', value: {label: 'Max', value: function(collection) { return collection.max(); }}},
  {label: 'Composite StdDev', value: {label: 'StdDev', value: function(collection) { return collection.reduce(ee.Reducer.stdDev()); }}}
];
...
exports.isValidDate = function(dateText) {
  ...
  return testDate.getDate() === day && testDate.getMonth() === (month - 1) && testDate.getFullYear() === year ? ee.Date(testDate) : false;
};
...
exports.createSceneSelectItems = function(imageCollection) {
  return imageCollection.map(function(scene) {
    var dict = {
      value: scene.id(),
      label: scene.date().format('dd/MM/YYYY'),
    };
    return ee.Feature(null, dict);
  }).getInfo().features.map(function(feature) {
...
exports.extractDateRange = function(image, option) {
  ...
  if (option === 'Month') {
    ...
    return ee.Feature(null, {
      label: monthYear,
      value: {label: monthYear, value: {option: monthYear, start: start, end: end}}
    });
    ...
    return ee.Feature(null, {
      label: quarterYear,
      value: {label: quarterYear, value: {option: quarterYear, start: start, end: end}}
    });
    ...
    semesterYear = ee.Algorithms.If(s2, ee.String('S2_').cat(year), semesterYear);
    start = ee.Algorithms.If(s2, ee.Date.fromYMD(year, 7, 1), start);
    end = ee.Algorithms.If(s2, ee.Date.fromYMD(year, 12, 31), end);
    
    return ee.Feature(null, {
      label: semesterYear,
      value: {label: semesterYear, value: {option: semesterYear, start: start, end: end}}
    });
  } else if (option === 'Year') {
    ...
    return ee.Feature(null, {
      label: yearLabel,
      value: {label: yearLabel, value: {option: yearLabel, start: start, end: end}}
    });
  }
};
...
exports.createDataFilters = function() {
  ...
  var startDateLabel = ui.Label({
    value: 'Start Date',
    style: {stretch: 'horizontal', textAlign: 'center', fontSize: '11px', margin: '10px 0px 2px 0px'}
  });
